...
- **Name the cure.** Draft: grade the contribution, not the patch — score the PR description's reasoning against the diff (their own maintainers could rubric it), or validate the "would merge" construct against a sample of real maintainer merge decisions. The strong form is the detector from [(Issue) → PR](https://june.kim/issue-to-pr): a classifier trained on a repo's actual merge/close corpus predicts the construct directly, consumes the whole contribution (diff, description, interaction), and recalibrates as the repo's floor moves — where a frozen rubric encodes one snapshot of 20 maintainers' taste. Ground truth for "would merge" exists in bulk in git history; a benchmark claiming to measure mergeability that never validates against it has skipped its own criterion validity check. Hold the cure to the same standard before proposing it (the detector's calibration would itself need receipts).
