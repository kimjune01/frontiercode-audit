...
2. The grading oracle is identified only as "an LLM": no model name, no version, no published pinning policy. Readers cannot assess judge drift or the judge's independence from the systems it grades (F7, oracle).
